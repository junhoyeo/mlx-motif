...
- `MLX_MOTIF_4SLOT_CACHE=1|q4|q8` selects grouped fp/q4/q8 cache implementations in Swift. The 4-slot fp16 cache is now the DEFAULT (unset -> on). q4/q8 decode passes packed tuples directly to `MotifSDPADualVQ4` (direct packed Metal dispatch); measured throughput parity vs Python is still the open gate.

Landed since PR #7 (see [`swift-full-parity-followup.md`](swift-full-parity-followup.md)): default-on direct custom Metal runtime, packed `sdpa_dual_v_q4`, real speculative decoding (batched `[1,K+1]` verification + lossless rejection sampling for temperature>0), tool-calling parity, KV-cache reuse across turns, full-capacity contiguous cache fetches with runtime `kv_len`, and a checked-in perplexity/long-context evidence harness. The one genuinely-open gate is **measured performance parity** against Python, which the checked-in local report does not yet prove.
