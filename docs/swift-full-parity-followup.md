...
| Tool / function calling | Implemented with Python parity | `MotifToolCalling` (`buildToolsPreamble` / `parseToolCall`) ports the Python prompt-based tool-calling protocol; `MotifNativeServe` re-reads the OpenAI `tools` array + `tool_choice: "none"` and injects the same deterministic preamble, surfacing parsed `tool_calls`. Covered by `MotifToolCallingTests`. Python side adds `run_tool_loop` + `mlx-motif tools-demo` for the execution loop. |
| KV-cache reuse across turns | Implemented | The native backend is persisted across turns so the KV cache is reused instead of rebuilt per request; covered by `MotifMLXNativeRuntimeCacheReuseTests`. |
| Speculative decoding | Implemented in Swift native path (greedy + lossless sampling) | `MotifMLXNativeRuntime.speculativeGenerate` and `MotifNativeGenerate --speculative --speculative-draft-model ...` run real draft-propose / batched-target-verify decoding: persistent draft and target KV caches, one `[1, K+1]` target forward per K-token draft block, cache `trim` past the first mismatch. Two lossless accept rules selected by temperature: greedy (`temperature == 0`) accepts a draft token iff it equals the target's greedy continuation; sampling (`temperature > 0`) uses standard rejection sampling (`MotifSpeculativeEngine.decodeSampling`) drawing corrections from the normalized residual. `MotifSpeculativeDecodingTests` asserts token-for-token equality with plain greedy decode and fewer target forwards than tokens. |
...
| Performance parity against Python | Optimized; residual measured and explained (roadmap stop-condition met via the "measured MLX-Swift gap" clause) | Decode was ~31% slower on the 2.6B; the dominant cause is fixed. The vanilla differential-attention path stored V in paired head order and re-copied the ENTIRE cached V (O(context)) every decode token, then ran a 2·d-wide generic SDPA; it now stores V slab-ordered at projection time (O(1) at decode) and issues 4 narrow fast-template SDPA calls, provably equivalent by linearity. **Decode 44 → ~53.6 tok/s (+22%)** at 3000 ctx, parity preserved (top-1 logit identical, perplexity 1.39270 vs 1.39147 = +0.09%). Clean A/B on both models showed the custom Metal kernels + 4-slot cache are decode-neutral (Swift PolyNorm kernel is a byte-for-byte port of Python's), and a node-by-node forward audit found no residual Motif-side inefficiency. The remaining ~16% (Swift ~53.6 vs Python-with-kernels 64) is per-token CPU overhead in the mlx-swift-lm generate loop + Swift-MLX bindings (GPU compute is identical) — not Motif code and not addressable in this repo. Full attribution: [issue #52](https://github.com/junhoyeo/mlx-motif/issues/52). Re-run parity fixtures with `scripts/test_mlx_runtime.sh`; re-run the sweep with `scripts/bench_sweep.py`. |
...
```bash
MLX_MOTIF_4SLOT_CACHE=1      # grouped four-slot fp cache (DEFAULT when unset; =0 opts out)
