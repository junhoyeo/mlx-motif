...
| 9 | **OpenAI-compatible endpoint** selected | Set **OpenAI-compatible endpoint** field to a syntactically invalid or non-HTTP URL (e.g. `not a url` or `ftp://example.com`) and send a prompt | Red error text: *"Endpoint must be an absolute HTTP or HTTPS URL with a host."*; **Runtime status** shows **"Error"** | | `ChatStore.swift` `MotifChatStoreError.invalidEndpoint`; endpoint text field in `ContentView.swift` |
