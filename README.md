...
| `MLX_MOTIF_FUSE_QKV` (Swift) | `1` | QKV-projection fusion for the grouped q4 decode path. Defaults ON; `0`/`off` opts out. Synthetic decode micro-benchmark (`MotifDecodeBench`, q4 gs=64, B=1, S=1) shows ~12-20% lower median ms/step at the 12.7B per-layer shape with fusion ON. This is a synthetic-weights per-decode-step measurement on one machine — **not** an end-to-end tok/s number; end-to-end validation on a real checkpoint is still required. |
