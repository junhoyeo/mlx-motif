...
| Sidebar | The **NavigationSplitView** sidebar shows a **Runtime** entry plus the conversation history. The chat pane is the default detail view; **File → New Chat (⌘N)** returns to it. All runtime settings live under **Runtime**. |
...
|---|--------------|--------|-----------------|-----------------|-------|
| 1 | App not yet running | Launch via the command above | Window appears; sidebar shows **Runtime** and the conversation history; the chat pane is active with an empty message list and the **"Ask Motif…"** text field visible | | `ContentView.swift` `NavigationSplitView` entry point; input field a11y id `motif.chat.input` |
...
| 9 | **OpenAI-compatible endpoint** selected | Set **OpenAI-compatible endpoint** field to a syntactically invalid or non-HTTP URL (e.g. `not a url` or `ftp://example.com`) and send a prompt | Red error text: *"Endpoint must be an absolute HTTP or HTTPS URL with a host."*; **Runtime status** shows **"Error"** | | `ChatStore.swift` `validatedEndpointURL` → `MotifChatStoreError.invalidEndpoint`. (`validatedEndpointURL` requires an http/https scheme + host precisely because on macOS 14+ Foundation's lenient URL parser percent-encodes garbage like `not a url` into *valid* URLs — raw `URL(string:) == nil` alone no longer catches it.) |
